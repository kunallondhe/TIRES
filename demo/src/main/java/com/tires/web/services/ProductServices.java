...
public class ProductServices {
	...
	/*
	 * public Boolean SaveDirectProduct(Product p) {
	 * 
	 * Boolean sucess = false; try {
	 * 
	 * ProductDao obj = new ProductDao();
	 * 
	 * if (p == null) { p = new Product();
	 * 
	 * p.setItemNumber(String.valueOf(ThreadLocalRandom.current().nextInt(555555,
	 * 999999))); p.setWidth(String.valueOf(ThreadLocalRandom.current().nextInt(210,
	 * 325))); p.setProfile(String.valueOf(ThreadLocalRandom.current().nextInt(35,
	 * 60))); p.setRimsize(String.valueOf(ThreadLocalRandom.current().nextInt(15,
	 * 22))); p.setLocalisedName("Michelin " + p.getWidth() + "/" + p.getProfile() +
	 * " R" + p.getRimsize() + " 92 (V) PILOT ALPIN 4 XL N0 Porsche");
	 * p.setDescription(
